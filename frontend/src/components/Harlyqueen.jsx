...
import { useHarleyStore } from "../store/useHarleystore";

function ChatInterface() {
  const [input, setInput] = useState("");
  const { generateContent, isGenerating, generatedResponse, clearResponse } = useHarleyStore();
      ...
      <div className="hidden lg:flex lg:w-1/3 bg-gradient items-center justify-center p-6">
        <div className="relative w-full h-full flex items-center justify-center">
          ...
          <div className="absolute bottom-8 text-center px-4">
            <h2 className="text-2xl font-bold text-white drop-shadow-lg">Harley Quinn</h2>
            <p className="text-white opacity-80 drop-shadow-md">A lovable lunatic with a PhD in mayhem and a knack for bad decisions!</p>
          </div>
        </div>
      </div>
        ...
        <div className="">
          <div className="flex-1">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold">Gemini AI Assistant</h1>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col p-4 max-w-4xl mx-auto w-full h-full">
          ...
          <div className="flex-1 overflow-y-auto mb-4 bg-base-100 rounded-box shadow-lg p-4 h-full">
            {filteredMessages.length > 0 ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-base-content opacity-50">
                ...
                <p className="text-lg font-medium">Start a conversation with Harley Quinn</p>
                <p className="text-sm mt-2">Ask anything from her</p>
              </div>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="join w-full">
            <div className="form-control join-item flex-1">
              <textarea
                className="textarea textarea-bordered w-full focus:outline-none focus:border-primary join-item rounded-l-lg rounded-r-none resize-none"
                value={input}
            ...
            <button
              ...
              className={`btn join-item ${isGenerating || !input.trim() ? 'btn-disabled' : 'btn-primary'}`}
            >
              {isGenerating ? (
